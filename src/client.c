    ...
    else
    {
        ...
        while (1)
        {
            ...
            if (totalBytes == (atoi(my_msg[2]) - atoi(tmp_size)))
            {
                fclose(fp);
                char hashCode[41];
                strcpy(hashCode, generate_sha1_of_file(save_file)); /* 本地计算接收文件的hashcode */
...
int client_upload_tcp(char* filename, char* singlefilename)
{
    ...
    /* 计算待发送文件的hashcode,打包发给服务端 */
    strcpy(my_fileInfo.hashCode, generate_sha1_of_file(my_fileInfo.fileName)); 
    ...
    printf("Send completed! buf size = %d bytes\n", completeSize);
    fclose(fp);
