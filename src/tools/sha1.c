#include "sha1.h" /* .c文件默认会寻找同名的.h文件，可以不加 */
...
char* generate_sha1_of_file(char * filename)
{
    ...
    /* 返回结果 静态数组的形式返回 */
    static char sha1_result[20][2] = {0};
    
    for (i = 0; i < 20; i++)
    {
        sprintf(sha1_result[i], "%02x", ctx.buf[i]);
    }
    //printf("%s",*sha1_result);
    return *sha1_result;
}
...
// {
    // char *a;
    // a = generate_sha1_of_file("test.txt");
    // printf("\n%s",a);
    // return 0;
// }
